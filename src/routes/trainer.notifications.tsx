...
export const Route = createFileRoute("/trainer/notifications")({
  head: () => ({
    meta: [
      { title: "Notifications · Trainer Portal · Capacity Connect" },
      {
        name: "description",
        content: "View and manage your trainer notifications and alerts.",
      },
    ],
  }),
  component: TrainerNotificationsPage,
});
...
function TrainerNotificationsPage() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      // 2. Fetch notifications for current trainer (or global ones where user_id is null)
      let query = supabase
        .from("notifications")
        .select("*")
    ...
    const channel = supabase
      .channel("public:trainer_notifications")
      .on(
        "postgres_changes",
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Stay updated with course submissions, feedback alerts, and administrative announcements.
          </p>
        </div>
